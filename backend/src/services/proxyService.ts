...
// 80% similarity threshold for fuzzy cache
...
interface ProxyCallOutput {
  response:    any;
  cacheHit:    boolean;
  cacheScore?: number;   // similarity score if fuzzy hit
  usage:       LLMUsageResult;
  latencyMs:   number;
  model:       string;
}
...
export const proxyService = {
  async call(input: ProxyCallInput): Promise<ProxyCallOutput> {
    ...
    // ── Extract keywords from incoming prompt ──────────────────
    const promptText      = extractPromptText(body);
    const incomingKeywords = [...extractKeywords(promptText)];

    // ── Step 1: Exact cache check ──────────────────────────────
    const exactCacheKey = buildCacheKey(provider, body);
    const exactHit      = await redisClient.getResponse(exactCacheKey);

    if (exactHit) {
      logger.debug({ org: ctx.organizationId }, 'Exact cache HIT');

      const wouldHaveCost = await pricingService.calculate(
        provider, model,
        exactHit.response?.usage?.prompt_tokens     ??
        exactHit.response?.usage?.input_tokens      ?? 0,
        exactHit.response?.usage?.completion_tokens ??
        exactHit.response?.usage?.output_tokens     ?? 0,
      );
    ...
    // ── Step 2: Fuzzy cache check (>= 80% similarity) ─────────
    const fuzzyHit = await redisClient.getFuzzyResponse(
      provider,
      model,
      incomingKeywords,
      FUZZY_THRESHOLD
    ...
    if (fuzzyHit) {
      logger.info(
        { score: fuzzyHit.similarity, org: ctx.organizationId },
        'Fuzzy cache HIT'
      );

      const wouldHaveCost = await pricingService.calculate(
        provider, model,
        fuzzyHit.cached.response?.usage?.prompt_tokens     ??
        fuzzyHit.cached.response?.usage?.input_tokens      ?? 0,
        fuzzyHit.cached.response?.usage?.completion_tokens ??
        fuzzyHit.cached.response?.usage?.output_tokens     ?? 0,
      );
      ...
      return {
        response:   fuzzyHit.cached.response,
        cacheHit:   true,
        cacheScore: fuzzyHit.similarity,
        usage:      ZERO_USAGE,
        latencyMs:  0,
        model,
      };
    ...
    // ── Step 3: Load + decrypt provider credential ─────────────
    const credential = await credentialRepository.findByOrgAndProvider(
      ctx.organizationId,
      provider
    );
    ...
    // ── Step 6: Store in cache with keywords ───────────────────
    redisClient
      .setResponse(
        exactCacheKey,
      ...
      .catch((err) =>
        logger.warn({ err }, 'Failed to write response to cache')
      );
